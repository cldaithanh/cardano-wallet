...
icaAddresses mw =
  let (seed, pwd) =
   ...
   in [ paymentAddress @'Mainnet $ publicKey $ addrXPrv ix
        | ix <- [minBound .. maxBound]
      ]
...
genRewardAccounts mw =
  let (seed, pwd) =
   ...
   in [getRawKey $ publicKey acctXPrv]
...
shelleyIntegrationTestFunds =
  ...
  where
    ...
    preregKeyWalletFunds =
      map
        ((,defaultAmt) . Address . unsafeFromHex)
