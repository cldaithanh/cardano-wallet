...
eventuallyUsingDelay delay timeout desc io = liftIO $ do
  ...
  case winner of
    Left () -> do
      ...
      let msg =
            "Waited longer than " ++ show timeout
              ++ "s to resolve action: "
              ++ show desc
              ++ "."
...
delegationFee ::
  forall w m.
  (HasType (ApiT WalletId) w, MonadIO m, MonadUnliftIO m) =>
  Context ->
  w ->
...
randomAddresses mw =
  let (seed, pwd) =
   ...
   in [ paymentAddress @n (publicKey $ addrXPrv ix)
...
getTransactionViaCLI ::
  ...
  String ->
  m r
getTransactionViaCLI ctx wid tid =
  cardanoWalletCLI $
    join
      [ ["transaction", "get"],
        ["--port", show (ctx ^. typed @(Port "wallet")), wid, tid]
...
replaceStakeKey ::
  forall (n :: NetworkDiscriminant).
  (DecodeAddress n, EncodeAddress n) =>
  (ApiT Address, Proxy n) ->
  (ApiT Address, Proxy n) ->
  (ApiT Address, Proxy n)
replaceStakeKey addr1 addr2 =
  let (hrp1, (tag1, pay1, _stake1)) = decodeAddr addr1
      (hrp2, (tag2, _pay2, stake2)) = decodeAddr addr2
   in if tag1 == tag2 && hrp1 == hrp2
  ...
  where
    ...
    decodeAddr =
      ...
      where
        ...
        splitAddr whole =
          let (tag, rest) = BS.splitAt 1 whole
              (paymentKeyH, stakeKeyH) = BS.splitAt 28 rest
           in case (BS.length tag, BS.length paymentKeyH, BS.length stakeKeyH) of
