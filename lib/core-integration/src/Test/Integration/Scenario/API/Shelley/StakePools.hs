...
spec = describe "SHELLEY_STAKE_POOLS" $ do
            ...
            pool : _ <-
                map (view #id) . snd
                    <$> unsafeRequest
                        ...
                        Empty
            joinStakePool @n ctx pool (w, "Wrong Passphrase")
                >>= flip
                    verify
            ...
            pool : _ <-
                map (view #id) . snd
                    <$> unsafeRequest @[ApiStakePool]
                        ...
                        (Link.listStakePools arbitraryStake)
                        Empty
            joinStakePool @n ctx pool (w, fixturePassphrase)
                >>= flip
                    verify
                    [ expectResponseCode HTTP.status202
                    , expectField (#status . #getApiT) (`shouldBe` Pending)
    ...
    it "STAKE_POOLS_JOIN_01 - Can rejoin another stakepool" $ \ctx -> runResourceT $ do
        ...
        joinStakePool @n ctx pool1 (w, fixturePassphrase)
            >>= flip
                verify
                [ expectResponseCode HTTP.status202
                , expectField (#status . #getApiT) (`shouldBe` Pending)
                    ...
                    ( \case
                        [dlg] -> do
                            ...
                            (view #epochNumber <$> dlg ^. #changesAt)
                                `shouldBe` Just (ApiT $ currentEpoch + 2)
                        _ ->
        ...
        joinStakePool @n ctx pool2 (w, fixturePassphrase)
            >>= flip
                verify
                [ expectResponseCode HTTP.status202
                , expectField (#status . #getApiT) (`shouldBe` Pending)
    ...
    describe "STAKE_POOLS_LIST_01 - List stake pools" $ do
        ...
        it "contains pool metadata" $ \ctx -> runResourceT $
            bracketSettings ctx $ do
                updateMetadataSource ctx "direct"
  ...
  where
    ...
    depositAmt ctx =
        let c = ctx ^. #_networkParameters . #protocolParameters . #stakeKeyDeposit
         in fromIntegral (unCoin c)
