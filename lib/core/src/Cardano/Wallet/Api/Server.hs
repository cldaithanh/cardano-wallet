...
postLedgerWallet ctx body = do
  ...
  where
    ...
    rootXPrv = Icarus.generateKeyFromHardwareLedger mw pwd
      where
        mw = getApiMnemonicT (body ^. #mnemonicSentence)
...
getWallet ctx mkApiWallet (ApiT wid) = do
  ...
  where
    ...
    whenNotResponding _ = Handler $
        ExceptT $
            withDatabase df wid $ \db -> runHandler $ do
...
decodeTransaction ctx (ApiT wid) (ApiSerialisedTransaction (ApiT sealed)) = do
        ...
        withWorkerCtx ctx wid liftE liftE $ \wrk -> do
            ...
            policyXPubM <-
                fmap (fmap fst . eitherToMaybe)
                    <$> liftIO . runExceptT
                    $ W.readPolicyPublicKey @_ @s @k @n wrk wid
            pure
                ( txinsOutsPaths
                , collsOutsPaths
                , outsPath
  ...
  where
    ...
    toApiAssetMintBurn xpubM tokenWithScripts =
        ApiAssetMintBurn
    ...
    toWrdl acct (rewardKey, (Coin c)) =
        if rewardKey == acct
            then ApiWithdrawalGeneral (ApiT rewardKey, Proxy @n) (Quantity $ fromIntegral c) Our
            else ApiWithdrawalGeneral (ApiT rewardKey, Proxy @n) (Quantity $ fromIntegral c) External
    ...
    toApiDelCert acct acctPath (W.CertDelegateNone rewardKey) =
        if rewardKey == acct
            ...
            else DelegationCertificate $ QuitPoolExternal (ApiT rewardKey, Proxy @n)
...
delegationFee ctx (ApiT wid) = do
    withWorkerCtx ctx wid liftE liftE $ \wrk -> liftHandler $ do
        w <-
            withExceptT ErrSelectAssetsNoSuchWallet $
...
listStakeKeys lookupStakeRef ctx (ApiT wid) = do
    withWorkerCtx ctx wid liftE liftE $ \wrk -> liftHandler $ do
        ...
        let takeFst (a, _, _) = a
        mourAccount <-
            fmap (fmap takeFst . eitherToMaybe)
                <$> liftIO . runExceptT
                $ W.readRewardAccount @_ @s @k @n wrk wid
        ourApiDelegation <-
            liftIO $
    ...
    withWorkerCtx ctx wid liftE liftE $ \wrk -> do
        case (testEquality (typeRep @s) (typeRep @shelley), withdrawal) of
            ...
            (Just Refl, Just (ExternalWithdrawal (ApiMnemonicT mw))) -> do
                ...
                wdrl <-
                    liftHandler (W.queryRewardBalance @_ wrk acct)
...
mkApiFee mDeposit minCoins (FeeEstimation estMin estMax) =
    ApiFee
...
instance IsServerError ErrSubmitTransaction where
    toServerError = \case
        ErrSubmitTransactionNoSuchWallet e ->
            (toServerError e)
                { errHTTPCode = 404
                , errReasonPhrase = errReasonPhrase err404
                }
...
instance IsServerError ErrWithdrawalNotWorth where
    toServerError = \case
        ErrWithdrawalNotWorth ->
            apiError err403 WithdrawalNotWorth $
                mconcat
...
instance IsServerError ErrAddCosignerKey where
    toServerError = \case
        ...
        ErrAddCosignerKey WalletAlreadyActive ->
            apiError err403 SharedWalletNotPending $
                mconcat
        ...
        ErrAddCosignerKey (NoSuchCosigner cred (Cosigner c)) ->
            apiError err403 SharedWalletNoSuchCosigner $
                mconcat
                    [ "It looks like you've tried to add a cosigner key to a "
                    , "shared wallet's "
                    , toText cred
                    , " template to a "
                    , "non-existing cosigner index: "
                    , pretty c
                    , "."
                    ]
        ...
        SelectionLimitReached e ->
            apiError err403 TransactionIsTooBig $
                mconcat
                    ...
                    , showT (length $ view #inputsSelected e)
                    , " inputs."
                    ]
...
instance IsServerError ErrUpdateSealedTx where
    toServerError = \case
        ErrExistingKeyWitnesses {} ->
            apiError err400 ExistingKeyWitnesses $
                T.unwords
