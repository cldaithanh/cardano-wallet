...
toApiAsset ::
    ...
    W.AssetId ->
    ApiAsset
toApiAsset metadata_ (W.AssetId policyId_ assetName_) =
    ApiAsset
...
instance
    (DecodeStakeAddress n, DecodeAddress n) =>
    FromJSON (ApiCoinSelection n)
    where
    parseJSON = genericParseJSON defaultRecordTypeOptions
instance
    (EncodeStakeAddress n, EncodeAddress n) =>
...
instance {-# OVERLAPS #-} DecodeAddress n => FromJSON (ApiT Address, Proxy n) where
    parseJSON x = do
        let proxy = Proxy @n
        addr <-
            parseJSON x
                >>= eitherToParser
                    ...
                    . decodeAddress @n
        return (addr, proxy)
instance {-# OVERLAPS #-} EncodeAddress n => ToJSON (ApiT Address, Proxy n) where
...
instance
    ...
    FromJSON (ApiMintBurnTransaction n)
    where
    parseJSON = genericParseJSON defaultRecordTypeOptions
...
instance
    ...
    ToJSON (ApiMintBurnTransaction n)
    where
    toJSON = genericToJSON defaultRecordTypeOptions
    ...
    parseJSON obj = do
        derPathM <-
            ( withObject "ApiTxOutputGeneral" $
                    \o -> o .:? "derivation_path" :: Aeson.Parser (Maybe (NonEmpty (ApiT DerivationIndex)))
                )
                obj
        case derPathM of
            Nothing -> do
...
instance
    (DecodeAddress n, PassphraseMaxLength s, PassphraseMinLength s) =>
...
instance (DecodeAddress n) => FromJSON (ApiPutAddressesData n) where
    parseJSON = genericParseJSON defaultRecordTypeOptions
...
type instance
    PostMintBurnAssetDataT (n :: NetworkDiscriminant) =
...
type instance
    ApiWalletMigrationPostDataT (n :: NetworkDiscriminant) (s :: Symbol) =
        ApiWalletMigrationPostData n s

type instance
    ApiMintBurnTransactionT (n :: NetworkDiscriminant) =
