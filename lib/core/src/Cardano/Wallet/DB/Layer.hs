...
newDBLayerWith _cacheBehavior _tr ti SqliteContext{runQuery} = do
    ...
    return DBLayer
        ...
        , putTxHistory = \wid txs -> ExceptT $ do
            selectWallet wid >>= \case
                Nothing -> pure $ Left $ ErrNoSuchWallet wid
                Just _ -> modifyDBMaybe transactionsDBVar $ \(_txsOld, ws) ->
                    let
        ...
        , removePendingOrExpiredTx = \wid txId -> ExceptT $ do
            selectWallet wid >>= \case
                Nothing -> pure $ Left
                    $ ErrRemoveTxNoSuchWallet $ ErrNoSuchWallet wid
                Just _ -> do
                    ex <- modifyDBMaybe transactionsDBVar
                        $ \(TxHistoryF txsOld, _ws) ->
                            if TxId txId `Map.member` txsOld
                                then
                                    ...
                                    $ ErrNoSuchTransaction wid txId)
                    pure $ case ex of
                        Left ert -> case ert of
                            ErrRemoveTxNoSuchWallet _ ->
