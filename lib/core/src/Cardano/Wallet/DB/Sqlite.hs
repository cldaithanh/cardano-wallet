    ...
    return
        DBLayer
            ...
            , putLocalTxSubmission = \wid txid tx sl -> ExceptT $ do
                let errNoSuchWallet =
                        ErrPutLocalTxSubmissionNoSuchWallet $
                            ErrNoSuchWallet wid
                let errNoSuchTx =
                        ErrPutLocalTxSubmissionNoSuchTransaction $
...
readWalletDelegation ti wid epoch
    ...
    | otherwise = do
        ...
        active <-
            maybe W.NotDelegating toWalletDelegationStatus
...
readDelegationCertificate wid filters =
    fmap entityVal
...
metadataFromEntity walDelegation wal =
    W.WalletMetadata
    ...
    deleteLoose t =
        flip rawExecute [] $
            "DELETE FROM " <> t <> " WHERE tx_id IN ("
                ...
                <> t
                <> " "
                <> "LEFT OUTER JOIN tx_meta ON tx_meta.tx_id = "
                <> t
                <> ".tx_id "
...
updatePendingTxForExpiryQuery wid tip = do
    ...
    dbChunkedFor @LocalTxSubmission
        ( \batch ->
            deleteWhere
