...
nextChangeIndex (SeqAddressPool pool) (PendingIxs ixs) =
    let poolLen = AddressPool.size pool
        ...
        (ix, ixs') = case ixs of
            ...
            h : _
                | length ixs < AddressPool.gap pool ->
                    (succ h, PendingIxs (succ h : ixs))
            h : q ->
                (h, PendingIxs (q ++ [h]))
     in invariant
            "index is within first unused and last unused"
            (ix, ixs')
...
decoratePath SeqState {derivationPrefix} r ix =
    NE.fromList
            ...
            Right addr ->
                let (internal, !int) = lookupAddress addr (internalPool st)
                    ...
                    ours = case (external, internal) of
                        ...
                        _ -> Nothing
                 in ( ixs' `deepseq` ours `deepseq` ours
                    , st
                        { internalPool = int
                        , externalPool = ext
...
instance KnownNat p => IsOurs (SeqAnyState n k p) Address where
    isOurs (Address bytes) st@(SeqAnyState inner)
        | crc32 bytes < p =
            let pool = getPool $ externalPool inner
