...
selectionSkeleton s = SelectionSkeleton
    ...
    , skeletonAssetsToBurn = view #assetsToMint s
    }
...
performSelectionEmpty performSelectionFn constraints params =
    fmap adjustResult <$> performSelectionFn constraints (adjustParams params)
  where
    adjustParams
    ...
    adjustParams
        = over #extraCoinSource
            (adjust (`Coin.addCoin` minCoin) (const id))
        . over #outputsToCover
            (adjust (const (dummyOutput :| [])) (const . id))
    ...
    adjustResult
        = over #extraCoinSource
            (adjust (`Coin.difference` minCoin) (const id))
        . over #outputsCovered
            (adjust (const []) (const . F.toList))

    adjust :: a -> (NonEmpty TxOut -> a) -> a
    adjust x y = maybe x y $ NE.nonEmpty $ view #outputsToCover params
