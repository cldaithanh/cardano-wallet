...
--
-- If a transaction has not failed script validation, then the following
-- property should hold:
--
-- prop> balance (utxoFromTx tx) = foldMap tokens (outputs tx)
--
-- If a transaction has failed script validation, then the following property
-- should hold:
--
...
--
-- This function ignores the transaction's script validation status.
--
