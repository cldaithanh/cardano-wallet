...
genPtrCoverage (Ptr slotNo ix1 ix2) =
  checkCoverage $
    conjoin
...
genValueForTxOutCoverage val =
  let valList = map fst $ valueToList val -- includes ada
...
genTxOutCoverage era (TxOut addr val datum) =
  checkCoverage $
    conjoin
      [ genAddressInEraCoverage era addr,
        genTxOutValueCoverage era val,
    ...
    conjoin
      [ Map.size costModel == Map.size (fromJust Plutus.defaultCostModelParams)
        ...
        checkCoverage $
          cover
            1
            (elem 0 $ Map.elems costModel)
            "model param is zero"
            $ cover
              30
              (any (> 0) $ Map.elems costModel)
