    ...
    malformed = jsonValid ++ jsonInvalid
      where
        ...
        exampleCosignerXPub =
            ...
            \423856bc91c49e928f6f30f4e8d665d53eb4ab6028bd0ac971809d514c92db1" ::
                Text
        jsonValid =
            first (BodyParam . Aeson.encode)
                <$> [
...
instance Malformed (BodyParam (PostTransactionOldData ( 'Testnet pm))) where
    malformed = jsonValid ++ jsonInvalid
      where
        ...
        jsonValid =
            first (BodyParam . Aeson.encode) <$> paymentCases
                ++ [
                     -- passphrase
        ...
        jsonValid =
            first (BodyParam . Aeson.encode)
                ...
                , "passphrase": #{nameTooLong}
                    ...
                    ,
                        ( [aesonQQ|
                { "mint_burn": [{ "asset_name": "not hexadecimal"
                                , "operation": [ { "mint": { "receiving_address": #{addrPlaceholder}
                                                            , "amount": { "unit": "assets"
                                                                        , "quantity": 3
                                                                        }
