                                                              ...
                                                              cover
                                                                ...
                                                                $ cover
                                                                  ...
                                                                  $ prop_performSelection mockConstraints params $ \result ->
                                                                    cover
                                                                      10
                                                                      (selectionUnlimited && selectionSufficient result)
                                                                      "selection unlimited and sufficient"
                                                                      . cover
                                                                        2
                                                                        (selectionLimited && selectionSufficient result)
                                                                        "selection limited but sufficient"
                                                                      . cover
                                                                        2
                                                                        (selectionLimited && selectionInsufficient result)
...
prop_makeChange_success_delta ::
  ...
  Property
prop_makeChange_success_delta p change =
  let totalOutputWithChange =
        TokenBundle.add
          totalOutputValue
          (F.fold change)

      delta = TokenBundle.unsafeSubtract totalInputValue totalOutputWithChange
   in (delta === TokenBundle.fromCoin (view #requiredCost p))
  ...
  let deltaCoin =
        TokenBundle.getCoin $
          TokenBundle.unsafeSubtract
            totalInputValue
            totalOutputValue
   in deltaCoin < view #requiredCost p
...
prop_splitBundlesWithExcessiveTokenQuantities_length input maxQuantityAllowed =
  maxQuantityAllowed > TokenQuantity.zero ==> checkCoverage $
    property $
      cover
        5
        (lengthOutput > lengthInput)
