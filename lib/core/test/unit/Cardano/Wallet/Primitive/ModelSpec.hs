...
prop_applyBlockBasic s =
  ...
  where
    ...
    prop =
        let (_, cp0) = initWallet @_ block0 s
...
instance Semigroup WalletState where
    (WalletState ours a) <> (WalletState ours' b) =
        invariant
            "Semigroup WalletState must be defined on same addresses"
...
unit_applyTxToUTxO_spends_input tx txin txout coin =
    let tx' =
     ...
     in applyTxToUTxO tx' (UTxO $ Map.fromList [(txin, txout)])
