...
prop_filter_conjoin f b =
    let as = TokenMap.getAssets $ TokenMap.filter (applyFun f . AssetIdF) b
     in Set.foldr ((&&) . applyFun f . AssetIdF) True as === True
...
prop_filter_partition f b =
    let l = TokenMap.filter (applyFun f . AssetIdF) b
...
prop_filter_twice f b =
    let once = TokenMap.filter (applyFun f . AssetIdF) b
...
prop_difference_add x y =
    let delta = x `difference` y
     ...
     in counterexample ("x - y = " <> show delta) $
