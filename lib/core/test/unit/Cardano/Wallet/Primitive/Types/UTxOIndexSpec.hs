    ...
    checkCoverage $
        property $
            cover
                ...
                $ cover
                    20
                    (category == MatchWithAdaOnly)
                    "WithAdaOnly"
                    $ cover
                        20
                        (category == MatchWithAsset)
                        "WithAsset"
                        $ cover
                            20
                            (category == MatchWithAssetOnly)
                            "WithAssetOnly"
                            True
  where
...
prop_selectRandom_singleton selectionFilter u b = monadicIO $ do
  ...
  where
    ...
    expected = case selectionFilter of
        ...
        WithAdaOnly
            | tokenBundleIsAdaOnly b ->
                Just ((u, b), UTxOIndex.empty)
        WithAdaOnly ->
            Nothing
        WithAsset a
            | tokenBundleHasAsset b a ->
                Just ((u, b), UTxOIndex.empty)
        WithAsset _ ->
            Nothing
        WithAssetOnly a
            | tokenBundleHasAssetOnly b a ->
                Just ((u, b), UTxOIndex.empty)
        WithAssetOnly _ ->
            Nothing
...
prop_selectRandom_one_any i = checkCoverage $
    monadicIO $ do
...
prop_selectRandom_one_withAdaOnly i = checkCoverage $
    monadicIO $ do
...
prop_selectRandom_one_withAsset i a = checkCoverage $
    monadicIO $ do
        result <- run $ UTxOIndex.selectRandom i (WithAsset a)
        monitor $
            cover
                50
                (a `Set.member` UTxOIndex.assets i)
                "index has the specified asset"
        monitor $
            cover
                50
                (Set.size (UTxOIndex.assets i) > 1)
...
prop_selectRandom_one_withAssetOnly i a = checkCoverage $
    monadicIO $ do
...
prop_selectRandom_all_any i = checkCoverage $
    monadicIO $ do
        (selectedEntries, i') <- run $ selectAll Any i
        monitor $
            cover
                90
                (not (null selectedEntries))
...
prop_selectRandom_all_withAdaOnly i = checkCoverage $
    monadicIO $ do
...
prop_selectRandom_all_withAsset i a = checkCoverage $
    monadicIO $ do
...
prop_selectRandom_all_withAssetOnly ::
    UTxOIndex TestUTxO -> AssetId -> Property
prop_selectRandom_all_withAssetOnly i a = checkCoverage $
    monadicIO $ do
        ...
        assert $
            all
                ( \(_, b) ->
                    not (tokenBundleHasAssetOnly b a)
                )
                (UTxOIndex.toList i')
        assert $
            all
        ...
        forAll (genAssetId `suchThat` (/= a1)) $ \a2 ->
            checkCoverage $
                monadicIO $ do
                    ...
                    result <-
                        run $
                            UTxOIndex.selectRandomWithPriority i $
