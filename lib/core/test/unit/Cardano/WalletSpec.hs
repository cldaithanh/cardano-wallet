...
spec :: Spec
spec = parallel $
    describe "Cardano.WalletSpec" $ do
        ...
        describe "WalletLayer works as expected" $ do
            it
                "Wallet upon creation is written down in db"
                (property walletCreationProp)
            it
                "Wallet cannot be created more than once"
                (property walletDoubleCreationProp)
            it
                "Wallet after being created can be got using valid wallet Id"
                (property walletGetProp)
            it
                "Wallet with wrong wallet Id cannot be got"
            ...
            it
                "Can't change name if wallet doesn't exist"
        ...
        describe "Join/Quit Stake pool properties" $ do
            it
                "You can quit if you cannot join"
...
prop_guardQuitJoin (NonEmpty knownPoolsList) dlg rewards wdrl =
    let knownPools = Set.fromList knownPoolsList
     in let noRetirementPlanned = Nothing
         in case W.guardQuit dlg wdrl (Coin.fromWord64 rewards) of
                Right () ->
...
prop_localTxSubmission tc = monadicIO $ do
    ...
    res <- run $
        runTest st $ \ctx@(TxRetryTestCtx DBLayer {..} _ _ wid) -> do
            -- Test setup
            atomically $ do
...
instance {-# OVERLAPS #-} Arbitrary (ShelleyKey 'RootK XPrv, Passphrase "encryption") where
    shrink _ = []
    arbitrary = do
        pwd <- arbitrary
        mw <- arbitrary
