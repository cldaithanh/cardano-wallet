...
toShelleyBlockHeader genesisHash blk =
  let ShelleyBlock (SL.Block (SL.BHeader header _) _) headerHash = blk
   in W.BlockHeader
        { slotNo =
...
fromShelleyBlock gp blk@(ShelleyBlock (SL.Block _ (SL.TxSeq txs')) _) =
  let (txs, certs, _, _) = unzip4 $ map fromShelleyTx $ toList txs'
      certs' = mconcat certs
   in ( W.Block
...
fromAllegraBlock gp blk@(ShelleyBlock (SL.Block _ (SL.TxSeq txs')) _) =
  let (txs, certs, _, _) = unzip4 $ map fromAllegraTx $ toList txs'
      certs' = mconcat certs
   in ( W.Block
          { header = toShelleyBlockHeader (W.getGenesisBlockHash gp) blk,
            transactions = txs,
...
fromMaryBlock gp blk@(ShelleyBlock (SL.Block _ (SL.TxSeq txs')) _) =
  let (txs, certs, _, _) = unzip4 $ map fromMaryTx $ toList txs'
      certs' = mconcat certs
   in ( W.Block
          { header = toShelleyBlockHeader (W.getGenesisBlockHash gp) blk,
            transactions = txs,
  ...
  W.ProtocolParameters
    { decentralizationLevel =
      ...
      desiredNumberOfStakePools =
        desiredNumberOfStakePoolsFromPParams pp,
      minimumUTxOvalue =
        MinimumUTxOValueCostPerWord
          . toWalletCoin
          $ Alonzo._coinsPerUTxOWord pp,
      stakeKeyDeposit = stakeKeyDepositFromPParams pp,
      eras = fromBound <$> eraInfo,
...
localNodeConnectInfo sp net = LocalNodeConnectInfo params net . nodeSocketFile
  where
    params = CardanoModeParams (getCardanoEpochSlots sp)

-- | Convert genesis data into blockchain params and an initial set of UTxO
fromGenesisData ::
  forall e crypto.
  (Era e, e ~ SL.ShelleyEra crypto) =>
  ShelleyGenesis e ->
...
fromAlonzoTxBodyAndAux bod mad wits =
  ...
  where
    ...
    fromAlonzoScriptMap anyScript =
      if Map.filter isPlutusScript anyScript == Map.empty
        then
...
toCardanoSimpleScript = \case
  ...
  ActiveFromSlot slot ->
    Cardano.RequireTimeAfter
      Cardano.TimeLocksInSimpleScriptV2
