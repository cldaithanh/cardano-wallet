              ...
              return $
                NodeToClientProtocols
                  { localChainSyncProtocol =
                    ...
                    localStateQueryProtocol =
                      let tr' = contramap (MsgLocalStateQuery TipSyncClient) tr
                          codec = cStateQueryCodec $ (serialisedCodecs v) cfg
                       in InitiatorProtocolOnly $
                            MuxPeerRaw $
                              \channel ->
...
mkLocalTxSubmissionClient _tr localTxSubmissionQ =
  LocalNodeClientProtocols
    { localChainSyncClient = NoLocalChainSyncClient,
...
handleIOException ::
  ...
  -- | 'True' = retry on 'ResourceVanishedError'
  Bool ->
  IOException ->
  IO RetryAction
...
handleMuxError ::
  ...
  MuxError ->
  IO RetryAction
handleMuxError tr onResourceVanished =
  pure . errorType >=> \case
...
data Log where
  ...
  MsgAccountDelegationAndRewards ::
    forall era crypto.
    (Map (SL.Credential 'SL.Staking era) (SL.KeyHash 'SL.StakePool crypto)) ->
...
onAnyEra ::
  ...
  LSQ (CardanoBlock StandardCrypto) m a
onAnyEra onByron onShelley onAllegra onMary onAlonzo =
  currentEra >>= \case
