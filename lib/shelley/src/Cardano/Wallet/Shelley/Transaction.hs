...
mkTx networkId payload ttl (rewardAcnt, pwdAcnt) addrResolver wdrl cs fees era = do
    ...
    let withResolvedInputs (tx, _, _, _) =
            tx
...
newTransactionLayer networkId =
    TransactionLayer
        ...
        , estimateSignedTxSize = \pp tx -> do
            ...
            pure $
                withShelleyBasedBody anyShelleyTx $ \_era body ->
        ...
        , computeSelectionLimit = \pp ctx outputsToCover ->
            let txMaxSize = getTxMaxSize $ txParameters pp
             in MaximumInputLimit $
...
_evaluateTransactionBalance tx pp utxo extraUTxO = do
    ...
    pure $
        withShelleyBasedBody shelleyTx $ \era bod ->
            let utxo' =
                    Map.fromList
                ...
                extraUTxO' =
                    Map.fromList
                        ...
                        $ extraUTxO
             in lovelaceFromCardanoTxOutValue $
...
updateSealedTx (cardanoTx -> InAnyCardanoEra _era tx) extraContent = do
  ...
  where
    ...
    modifyLedgerTx ::
        forall era crypto.
        (crypto ~ Crypto (Cardano.ShelleyLedgerEra era)) =>
        TxUpdate ->
        Cardano.TxBody era ->
        ...
        adjustBody (TxUpdate extraInputs extraCollateral extraOutputs feeUpdate) era body = case era of
            ShelleyBasedEraAlonzo ->
                body
                    ...
                    , Alonzo.collateral =
                        Alonzo.collateral body
                            <> Set.fromList (Cardano.toShelleyTxIn <$> extraCollateral')
                    , Alonzo.txfee =
                        modifyFee $ Alonzo.txfee body
                    }
            ShelleyBasedEraMary ->
                let ShelleyMA.TxBody inputs outputs certs wdrls txfee vldt update adHash mint = body
            ...
            ShelleyBasedEraShelley ->
                let Shelley.TxBody inputs outputs certs wdrls txfee ttl txUpdate mdHash = body
                    toTxOut = Cardano.toShelleyTxOut era . Cardano.toCtxUTxOTxOut . toCardanoTxOut era
                 in Shelley.TxBody
                        ( inputs
                            <> Set.fromList (Cardano.toShelleyTxIn <$> extraInputs')
...
dummySkeleton inputCount outputs =
    SelectionSkeleton
        { skeletonInputCount =
            inputCount
        , skeletonOutputs =
...
estimateNumberOfWitnesses (Cardano.TxBody txbodycontent) =
    let txIns = Cardano.txIns txbodycontent
     ...
     in -- FIXME [ADP-1515] Not all certificates require witnesses. Will
...
_assignScriptRedeemers (toAlonzoPParams -> pparams) ti resolveInput redeemers tx =
  ...
  where
    ...
    assignNullRedeemers alonzoTx = do
        (indexedRedeemers, nullRedeemers) <- fmap unzip $
            forM redeemers $ \rd -> do
    ...
    utxoFromAlonzoTx alonzoTx =
        let inputs = Alonzo.inputs (Alonzo.body alonzoTx)
         ...
         in Ledger.UTxO (Map.fromList utxo)
    ...
    evaluateExecutionUnits indexedRedeemers alonzoTx = do
        ...
        epochInfo <-
            hoistEpochInfo (left ErrAssignRedeemersPastHorizon . runIdentity . runExceptT)
    ...
    addScriptIntegrityHash ::
        forall era.
        (era ~ Cardano.ShelleyLedgerEra Cardano.AlonzoEra) =>
        AlonzoTx ->
        AlonzoTx
        ...
        Cardano.makeTransactionBody $
            Cardano.TxBodyContent
                ...
                , Cardano.txCertificates =
                    let -- It seems that passing Map.empty here works just fine.
                        witMap = Map.empty
                        ctx = Cardano.BuildTxWith witMap
                     in Cardano.TxCertificates certSupported certs ctx
