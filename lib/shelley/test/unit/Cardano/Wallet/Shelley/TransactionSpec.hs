...
instance Arbitrary (Cardano.AddressInEra Cardano.AlonzoEra) where
    arbitrary = genAddressInEra Cardano.AlonzoEra
...
prop_balanceTransactionUnresolvedInputs wallet (ShowBuildable partialTx') seed =
  ...
  where
    ...
    dropResolvedInputs (PartialTx tx (Cardano.UTxO utxo) redeemers) = do
        let inputs = Map.toList utxo
        ...
        let resolution' = Cardano.UTxO $ Map.fromList inputs'
        pure (PartialTx tx resolution' redeemers, dropped)
...
prop_balanceTransactionValid wallet (ShowBuildable partialTx') seed
    = withMaxSuccess 1000 $ do
        let combinedUTxO =
                ...
                <> (toCardanoUTxO testTxLayer walletUTxO [])
        ...
        case res of
            ...
            Left ErrBalanceTxConflictingInputResolution ->
                 label "input resolution conflicts with wallet utxo"
                    $ property True
