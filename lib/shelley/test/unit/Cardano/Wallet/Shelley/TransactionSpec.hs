...
spec = do
  ...
  describe "Sign transaction" $ do
    forAllEras
      ( \era ->
          it ("signTransaction adds reward account witness when necessary (" <> show era <> ")") $
            property (prop_signTransaction_addsRewardAccountKey era)
      )
    forAllEras
      ( \era ->
          it ("signTransaction adds extra key witness when necessary (" <> show era <> ")") $
            property (prop_signTransaction_addsExtraKeyWitnesses era)
      )
    forAllEras
      ( \era ->
          it ("signTransaction adds tx in witnesses when necessary (" <> show era <> ")") $
            property (prop_signTransaction_addsTxInWitnesses era)
      )
    forAllEras
      ( \era ->
          it ("signTransaction adds collateral witnesses when necessary (" <> show era <> ")") $
            property (prop_signTransaction_addsTxInCollateralWitnesses era)
      )
    forAllEras
      ( \era ->
          it ("signTransaction never removes witnesses (" <> show era <> ")") $
            property (prop_signTransaction_neverRemovesWitnesses era)
      )
    forAllEras
      ( \era ->
          it ("signTransaction never changes tx body (" <> show era <> ")") $
            property (prop_signTransaction_neverChangesTxBody era)
      )
    forAllEras
      ( \era ->
          it ("signTransaction preserves script integrity (" <> show era <> ")") $
...
prop_signTransaction_addsRewardAccountKey (AnyCardanoEra era) rootXPrv utxo wdrlAmt = do
  whenSupportedInEra Cardano.withdrawalsSupportedInEra era $
    \(supported :: Cardano.WithdrawalsSupportedInEra era) -> do
      let rootK :: (ShelleyKey 'RootK XPrv, Passphrase "encryption")
...
prop_signTransaction_addsExtraKeyWitnesses (AnyCardanoEra era) rootK utxo extraKeys = do
  whenSupportedInEra Cardano.extraKeyWitnessesSupportedInEra era $
    \(supported :: Cardano.TxExtraKeyWitnessesSupportedInEra era) -> do
      let keys :: (XPrv, Passphrase "encryption") -> Cardano.SigningKey Cardano.PaymentExtendedKey
...
utxoFromKeys keys utxoProp =
  let addresses :: [Address]
   ...
   in forAll (vectorOf (length txOuts) genTxIn `suchThat` isUnique) $ \txIns -> do
...
withBodyContent era modTxBody cont =
  forAllShow (genTxBodyContent era) showTransactionBody $ \txBodyContent -> do
    let txBodyContent' = modTxBody txBodyContent
...
decodeSealedTxSpec = describe "SealedTx serialisation/deserialisation" $ do
  ...
  xdescribe "Not implemented yet" $ do
    -- TODO: [ADP-919]
...
feeCalculationSpec = describe "fee calculations" $ do
  ...
  it "increasing mint increases tx size at least proportianally to asset names" $
    property $ \mints ->
      let assetNameLength = BS.length . unTokenName . tokenName
       ...
       in -- Larger asset names means more bytes in the tx which should
  ...
  it "increasing scripts increases fee at least proportionate to size of CBOR script" $
    property $ \scripts ->
      let -- Number of signatures required in the script
          ...
          marginalSize :: Integer
          marginalSize = sizeWith - sizeWithout
       in -- The entire script must be serialized when it is included in
...
feeEstimationRegressionSpec = describe "Regression tests" $ do
  it "#1740 Fee estimation at the boundaries" $ do
    ...
    result
      `shouldBe` Right (FeeEstimation requiredCost requiredCost)
  ...
  describe "Byron witnesses - mainnet" $ do
    ...
    it "1 input, 2 outputs" $ do
      ...
      calculateBinary net utxo outs chgs pairs
        `shouldBe` "83a40081825820000000000000000000000000000000000000000000000000\
    ...
    it "2 inputs, 3 outputs" $ do
      ...
      calculateBinary net utxo outs chgs pairs
        `shouldBe` "83a40082825820000000000000000000000000000000000000000000000000\
...
estimateMaxInputsTests cases = do
  ...
  describe ("estimateMaxNumberOfInputs for " <> k) $ do
    ...
    prop
      "more outputs ==> less inputs"
      (prop_moreOutputsMeansLessInputs @k)
    prop
      "bigger size  ==> more inputs"
    ...
    describe "effect of txMaxSize on coin selection" $ do
      ...
      let totalOutput tx =
            let (wtx, _, _, _) = decodeTx testTxLayer tx
             in F.foldMap (view (#tokens . #coin)) (view #outputs wtx)
        ...
        where
          ...
          simplifyUTxO =
            UTxO . uncurry Map.singleton
              . foldl1
                ( \(i, TxOut addr _val1) (_, TxOut _ _val2) ->
...
restrictResolution (PartialTx tx inputs redeemers) =
  let inputs' = flip filter inputs $ \(i, _, _) ->
        i `Set.member` inputsInTx (cardanoTx tx)
   in PartialTx tx inputs' redeemers
  where
...
shrinkTxBody (Cardano.ShelleyTxBody e bod scripts scriptData aux val) =
  tail
    [ Cardano.ShelleyTxBody e bod' scripts' scriptData' aux' val'
      | bod' <- prependOriginal shrinkLedgerTxBody bod,
        ...
        val' <-
          ...
          filter
            (/= val)
            [ Cardano.TxScriptValidity
                Cardano.TxScriptValiditySupportedInAlonzoEra
                Cardano.ScriptValid
            ]
    ]
  where
...
prop_posAndNegFromCardanoValueRoundtrip = forAll genSignedValue $ \v ->
  let (pos, neg) = posAndNegFromCardanoValue v
   in toCardanoValue pos <> (Cardano.negateValue (toCardanoValue neg)) === v
...
estimateSignedTxSizeSpec =
  describe "estimateSignedTxSize" $ do
    it "equals the binary size of signed txs" $
      property $ do
        forAllGoldens signedTxGoldens $ \hexTx -> do
          ...
          estimateSignedTxSize testTxLayer pparams tx
            `shouldBe` Just (TxSize $ fromIntegral $ BS.length bs)
  where
