...
prop_balanceTransactionUnresolvedInputs seed = do
    ...
    checkCoverage $ forAll (genTxFromUTxO Cardano.AlonzoEra walletUTxO) $ \tx -> do
    -- forAll (genTxForBalancing Cardano.AlonzoEra) $ \tx -> do
        let (Cardano.Tx (Cardano.TxBody content) _) = tx
        let inputs = (fromCardanoTxIn . fst) <$> Cardano.txIns content
            outputs = fromCardanoTxOut <$> Cardano.txOuts content
            resolvedInputs = (\(i,o) -> (i, o, Nothing)) <$> zip inputs outputs
            redeemers = []
            partialTx = PartialTx tx resolvedInputs redeemers
        forAll (dropResolvedInputs partialTx) $ \(partialTx', dropped) -> do
          ...
          cover 1 (isUnresolvedTxInsErr res) "unknown txins" $
              case res of
                  Right _
                      | null dropped
                        -> label "nothing dropped" $ property True
                      | otherwise
                        -> label "succeeded despite unresolved input" $ property True
                            -- let droppedSet =
