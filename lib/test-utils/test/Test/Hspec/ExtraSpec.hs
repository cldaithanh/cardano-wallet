...
itSpec :: Spec
itSpec = describe "Extra.it" $
    before_ (setEnv "TESTS_RETRY_FAILED" "y") $ do
...
aroundAllSpec = sequential $ do
    ...
    describe "Extra.aroundAll" $ do
        describe "trivial" $
            aroundAll (withMockResource resourceA) $ do
                it
        ...
        describe "basic" $
            aroundAll (withMVarResource resourceA) $ do
