...
let
  ...
  project = haskell-nix.cabalProject' ({ pkgs, ... }: {
    ...
    modules = [
      {
        packages.beam-migrate.ghcOptions = [ "-Wno-unused-record-wildcards" ];
        packages.freer-extras.ghcOptions = [ "-Wno-unused-record-wildcards" ];
        packages.beam-sqlite.ghcOptions = [ "-Wno-unused-record-wildcards" ];
      }
      {
